...
class SACAgent(BaseAgent):
    ...
    def update_critic(self, ob_no, ac_na, next_ob_no, re_n, terminal_n):
        ...
        q1,q2 = self.critic.forward(next_ob_no,action)
        q_current = torch.min(q1,q2)
        log_prob_pi = action_dist.log_prob(action)


        target = re_n + self.gamma*(1-terminal_n)*(q_current - alpha*log_prob_pi)
        critic_loss_q1 = self.critic.loss(q1,target)
        critic_loss_q2 =self.critic.loss(q2,target)
        self.critic.optimizer.zero_grad()
        loss  = critic_loss_q1 + critic_loss_q2
        loss.backward()
    ...
    def train(self, ob_no, ac_na, re_n, next_ob_no, terminal_n):
        ...
        if self.training_step % self.critic_target_update_frequency:
            critic_loss = soft_update_params(self.critic,self.critic_target,self.critic_tau)
