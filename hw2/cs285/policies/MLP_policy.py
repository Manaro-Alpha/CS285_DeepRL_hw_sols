...
class MLPPolicyPG(MLPPolicy):
    ...
    def update(self, observations, actions, advantages, q_values=None):
        ...
        J = torch.neg(torch.mean(torch.mul(log_pi,advantages)))
        ...
        if self.nn_baseline:
            ...
            #TODO
            target = normalize(q_values,0,1)
            target = ptu.from_numpy(target)
            baseline_loss = self.baseline_loss(self.baseline(observations).squeeze(),target)
